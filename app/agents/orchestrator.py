...
class FinanceResearchOrchestrator:
    ...
    def _run_agents(self, ticker: str, evidence: list[Evidence]) -> list[AgentOutput]:
        if self.config.enable_langgraph:
            try:
                return self._run_with_langgraph(ticker, evidence)
            except Exception as exc:  # noqa: BLE001
                self.logger.log("langgraph_fallback", {"reason": str(exc)})
        return [
            self.fundamental.run(ticker, evidence),
            self.technical.run(ticker, evidence),
            self.valuation.run(ticker, evidence),
            self.news.run(ticker, evidence),
        ]
    ...
    def _run_with_langgraph(self, ticker: str, evidence: list[Evidence]) -> list[AgentOutput]:
        ...
        def run_fundamental(state: S) -> S:
            return {"outputs": state["outputs"] + [self.fundamental.run(ticker, evidence)]}

        def run_technical(state: S) -> S:
            return {"outputs": state["outputs"] + [self.technical.run(ticker, evidence)]}

        def run_valuation(state: S) -> S:
            return {"outputs": state["outputs"] + [self.valuation.run(ticker, evidence)]}

        def run_news(state: S) -> S:
            return {"outputs": state["outputs"] + [self.news.run(ticker, evidence)]}
        ...
        graph.add_edge("technical", "valuation")
        graph.add_edge("valuation", "news")
